...
const AddToCartBtn = ({ className='', qty=1, product }: IProps) => {
    ...
    return isLogged ? 
        !isInCart ?
            <button type="button" className={`addToCartBtn ${className}`} onClick={() => dispatch<any>(onAddProduct([product._id, qty]))}>Add to cart</button>
            :
            <Link to='/bruden/cart' className={`addToCartBtn ${className}`}>Go to Cart</Link>
    :
    <Link to='/bruden/auth' className={`addToCartBtn ${className}`}>Add to cart</Link>
}
