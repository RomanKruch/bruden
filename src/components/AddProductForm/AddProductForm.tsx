...
import SelectListType from '../SelectListType/SelectListType';
import { useState, useEffect } from 'react';
import { useFormik } from 'formik';
import axios from "axios";
import ShopListItem from '../pages/ShopPage/sections/Shop/ShopListItem/ShopListItem';
...
const AddProductForm = () => {
    ...
    const formik = useFormik({
      ...
      onSubmit:  values => {
        ...
        .then(({data}) => setProduct({...data.data.product, tag: data.data.product.tag.name}))
  },
    });
        ...
        <form onSubmit={formik.handleSubmit} className='adminPage_form'>
            ...
            <label className='adminPage_label'>
                tag:
                    ...
                    >
                        {tags.map(item => (
                            <MenuItem key={item._id} value={item.name} className="sortBar_select_item">{item.name}</MenuItem>
                        ))}
                    </Select>
            </label>
            ...
            <input type="file" name="largeImg" onChange={(event) => {
                const file  = event.currentTarget.files ? event.currentTarget.files[0] : null;
                if (file) {
                formik.setFieldValue("largeImg", file);
                }
            }}/>
            <button type="submit" className='adminPage_btn'>Create</button>
        </form>
        {product && 
            <>
